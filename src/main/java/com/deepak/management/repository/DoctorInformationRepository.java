...
public interface DoctorInformationRepository extends JpaRepository<DoctorInformation, Long> {
     ...
     * "WHERE di.clinic_id = :clinicId AND di.doctor_id = :doctorId AND dai.absence_date >= CURDATE()"
     * , nativeQuery = true)
     * List<Object[]> getDoctorsWithCurrentDateAndDayOfWeek(@Param("doctorId")
     * String doctorId, @Param("clinicId") String clinicId);
     */
