 ...
 /*   @Query(value = "SELECT " +
    ...
    @Query(value =
            "SELECT " +
                    ...
                    "LEFT JOIN doctor_absence_information dai ON di.clinic_id = dai.clinic_id " +
                    "AND di.doctor_id = dai.doctor_id " +
                    "AND dai.absence_date = CURDATE() " +
                    "WHERE " +
                    "di.clinic_id = :clinicId " +
                    "AND di.doctor_id = :doctorId",
            nativeQuery = true)
    List<Object[]> getDoctorsWithCurrentDateAndDayOfWeek(
